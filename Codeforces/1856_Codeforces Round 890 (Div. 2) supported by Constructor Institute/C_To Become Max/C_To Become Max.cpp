...
bool helper(const VI& arr, const int& i, const int& k, const int& mid){
    if(i>=arr.size()||mid-arr[i]>k)return false;
    if(arr[i]>=mid)return true;
    return helper(arr, i+1, k-mid+arr[i],mid-1);
}

void solve(){
    int n, k, max=0, l, r, mid;
    cin>>n>>k; VI arr(n); cin>>arr;
    REP(n){
        r=(l=arr[i])+k;
        while(l<r){
            int mid =(l+r)/2+1;
            if(helper(arr,i,k,mid)) l=mid;
            else r = mid-1;
        }
        cmax(max,l);
    }
