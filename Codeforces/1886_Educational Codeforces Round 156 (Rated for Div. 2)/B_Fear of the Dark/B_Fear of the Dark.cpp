...
void solve(){
    double px, py, ax, ay, bx, by, a=inf, b=inf, c=inf, ca, cb;
    bool oc, pc;
    cin >> px >> py >> ax >> ay >> bx >> by;
    double mx = (ax+bx)/2, my = (ay+by)/2;
    double md = sqrtl((ax-mx)*(ax-mx) + (ay-my)*(ay-my));
    
    a = md; // midpt distance
    
    double doa = sqrtl((ax-0)*(ax-0) + (ay-0)*(ay-0));
    double dob = sqrtl((bx-0)*(bx-0) + (by-0)*(by-0));
    double dpa = sqrtl((ax-px)*(ax-px) + (ay-py)*(ay-py));
    double dpb = sqrtl((bx-px)*(bx-px) + (by-py)*(by-py));

    ca = max(doa, dpa);
    cb = max(dob, dpb);
    b = min(ca, cb); // w for both fitting in same circle

    ca = min(doa, dob);
    cb = min(dpb, dpa);
    c = max(ca, cb); // w for both fitting in different circle 
