...
void solve(){
    int n, k, pos=0, a=0, b=0; cin>>n>>k;
    VI arr(n), counts(n+1,0); 
    REP(n){
        cin>>arr[i];
        counts[arr[i]]++;
    }
    if(arr[0]==arr[n-1]){
        if(counts[arr[0]]<k) NO();
        else YES();
        return;
    }

    if(counts[arr[0]]<k || counts[arr[n-1]]<k){
        NO(); return;
    }

    while(a<n&&pos<k) if(arr[a++]==arr[0]) pos++;
    for(pos=0, b=n-1;b>=a;b--) pos+=(arr[n-1]==arr[b]);
    if(pos>=k) YES();
    else NO();
}