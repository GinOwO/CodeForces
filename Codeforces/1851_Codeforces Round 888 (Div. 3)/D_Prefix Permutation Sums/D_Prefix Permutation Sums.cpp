...
void solve(){
    int n, dif, pos=0, a; cin>>n;
    VI arr(n-1); SI st; st.insert(n);
    REP(n-1){
        cin>>arr[i];
        st.insert(i+1);
    }
    if(st.count(arr[0])) st.erase(arr[0]), a=1;
    else a=0, pos=arr[0];

    REPk(1,n-1){
        dif = arr[i]-arr[i-1];
        if(st.count(dif)) st.erase(dif);
        else{
            if(!pos) pos=dif;
            else{
                NO();
    ...
    int sum=0;  FOR(c,st) sum+=c;
    if((!pos && (n*(n+1)/2-arr[n-2]==sum)) || (sum==pos)) YES();
    else NO();
