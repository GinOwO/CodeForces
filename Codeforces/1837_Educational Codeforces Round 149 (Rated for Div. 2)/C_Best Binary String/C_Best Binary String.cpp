...
void solve(){
    string s, out; cin>>s;
    VII domains; int start=-1, n=s.size();
    REP(n) {
        if (s[i] == '?') {
            if (start == -1) start = i;
        } 
        else if (start != -1) {
            domains.push_back({start, i});
            start = -1;
        }
    }

    if (start != -1) {
        domains.push_back({start, n});
    }
    int t = 0;
    REP(n){
        if (t<domains.size() && i == domains[t].first){
            int f = domains[t].first, se = domains[t].second;
            t++;
            if (i==0){
                for(;i<se;i++) out += "0";
                i--;
