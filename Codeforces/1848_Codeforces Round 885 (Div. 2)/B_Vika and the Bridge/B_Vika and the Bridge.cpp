...
bool canCrossBridge(const vector<int>& planks, int maxPlanks, int maxRepaint) {
    int count = 0; // Number of planks that need repainting
    int prevColor = planks[0];

    for (int i = 1; i < planks.size(); i++) {
        if (planks[i] != prevColor) {
            count++;
            if (count > maxRepaint) {
                return false;
            }
