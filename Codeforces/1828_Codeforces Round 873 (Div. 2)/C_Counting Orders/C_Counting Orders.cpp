...
long long merge(vector<int>& arr, vector<int>& b, int start, int mid, int end) {
    int i = start;
    int j = mid + 1;
    int k = start;
    long long count = 0;
    ...
    while (i <= mid && j <= end) {
        if (arr[i] > b[j]) {
            count += (mid - i + 1);  // Increment the inversion count
            temp[k++] = arr[j++];
        } else {
            temp[k++] = arr[i++];
        }
...
long long mergeSortCount(VI& arr, VI& b, int start, int end) {
    if (start >= end) {
        return 0;  // Base case: subarray has only one element
    }

    int mid = (start + end) / 2;

    long long leftCount = mergeSortCount(arr, b, start, mid);
    long long rightCount = mergeSortCount(arr, b, mid + 1, end);
    long long mergeCount = merge(arr, b, start, mid, end);
