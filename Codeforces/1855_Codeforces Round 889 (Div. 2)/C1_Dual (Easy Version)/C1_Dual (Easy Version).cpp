...
void solve(){
    int n, big=0, small=0, sorted=1; cin>>n;
    VI arr1(n),arr2(n); cin>>arr1; 
    VF(PII) ans, ans2; int a, b, p, q;
    REP(n) {
        if(arr1[big]<arr1[i]) big=i;
        if(arr1[small]>arr1[i]) small=i;
        arr2[i]=arr1[i];
        if(i>0 && arr1[i-1]>arr1[i]) sorted=0;
    }
    if(small == big || sorted){
        cout << "0\n";
        return;
    }
    int small2=small, big2=big;
    REPk(1,n){
        if(arr1[i-1]>arr1[i]){
            a=arr1[i-1]; b=arr1[i];
            p=ceil(abs((a-b)/arr1[small]));
            q=ceil(abs((a-b)/arr1[big]));
            if (p < q){
                arr1[i] += p*arr1[small];
                ans.push_back(PII{i+1,small+1});
                if(arr1[i]<arr1[small]) small=i;
            }
            else if(p > q){
                arr1[i-1] += q*arr1[big];
                ans.push_back(PII{i+1,big+1});
                if(arr1[i-1]>arr1[big]) big=i-1;
            }
        }
    }
    for(int i=n-1; i>0; i--){
        if(arr2[i]<arr2[i-1]){
            a=arr2[i-1]; b=arr2[i];
            p=ceil(abs((a-b)/arr2[small2]));
            q=ceil(abs((a-b)/arr2[big2]));
            if (p < q){
                arr2[i-1] += p*arr2[small2];
                ans2.push_back(PII{i,small2+1});
                if(arr2[i-1]<arr2[small2]) small2=i-1;
            }
            else if(p > q){
                arr2[i] += q*arr2[big2];
                ans2.push_back(PII{i+1,big2+1});
                if(arr1[i]>arr1[big2]) big2=i;
            }
        }
